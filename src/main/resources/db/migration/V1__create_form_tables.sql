...
(
    ...
    updated_at        TIMESTAMP   NULL,
    PRIMARY KEY (id),
    CONSTRAINT chk_fqu_input_type CHECK (input_type IN
                                         ('TEXT', 'NUMBER', 'EMAIL', 'PHONE', 'DATE', 'TIME', 'DATETIME_LOCAL',
                                          'NONE')),
    CONSTRAINT chk_fqu_question_type CHECK (question_type IN
                                            ('SINGLE_LINE_INPUT', 'CHECKBOXES', 'RADIO_BUTTON_GROUP', 'RATING_SCALE',
                                             'YES_NO', 'LONG_TEXT', 'FILE_UPLOAD', 'SINGLE_CHOICE_GRID',
